...
- 下方 JSON 仅为示例结构，用于说明数据格式，而非固定模板。
...
动态生成规则：
- 小节标题必须依据 requirement.json 与 toc 上下文自动生成。
- 若文档涉及设备清单、接口点位、测试用例、参数矩阵等结构化信息：**需要自行判断是否真的适合表格**（避免“为了形式堆表格”）。
  ...
  - 每个三级节点表格数量建议 <=2，除非确有大量结构化数据且能显著提升可读性。
- Style Extractor 提取的“结构偏好”可影响小节数量与命名方式。
- Layout Agent 只负责排版，不得更改生成的小节逻辑。
  ]
}
...
- 正文样式强制规范：**所有正文/图片段落/图题段落**必须显式使用 `standard_template.docx` 中的“正文/Normal（或项目约定正文样式）”，禁止出现未设样式的段落导致行距漂移（1倍/2倍混用）。
- 图宽：5英寸，居中
- 图题：居中，自动编号（图X-Y）
...
- 表格使用原则：仅在确有结构化多条数据（清单/矩阵/记录表）时使用；禁止把叙述性内容大面积塞进表格导致“表格过多、可读性下降”。
- 表头加粗；列宽自适应或按模板固定
- 表格标题：表X-Y 居中或左对齐（统一）
...
TC-15a 正文段落行距/段前段后统一继承 `standard_template.docx` 的“正文”样式（避免 1 倍/2 倍混用）
...
补充实现约束（防止漏插图）：
- 即使正文生成的 AST 中未显式包含 `image_grid` block，Layout Agent 也必须兜底把 `images.json` 中的已生成图片插入到 Word（优先按 `bind_anchor` 就近插入，否则插入到本节点标题之后）。
...
## 1.5 核心模块一：Style Extractor（内容逻辑与风格提取）

系统在任务开始阶段会生成 `style_profile.json`，所有内容生成 Agent 必须调用该文件。
...
**表格触发机制**
当内容包含以下类型数据时，优先考虑生成 Word 表格，但必须满足“结构化阈值”（否则用正文段落表达）：
- 阈值建议：>=3 行数据（不含表头）或 >=4 列，或明显是 2x2 以上参数矩阵。
...
表格数据输出为 JSON，由 Layout Agent 渲染。
