...
class RequirementParserAndTOCTestCase(unittest.TestCase):
    ...
    def test_toc_generator_uses_model_output_instead_of_template(self) -> None:
        ...
        self.assertTrue(all(node.is_generation_unit for node in root.children[0].children))
          ...
          "chapters": [
            {
              ...
              "children": [
                {"title": "2.1 项目范围与总体承包要求", "children": []},
                {"title": "2.2 技术标准与规范体系", "children": []}
              ]
            }
          ]
        }
    ...
    def test_toc_generator_strips_clause_number_prefix_from_leaf_titles(self) -> None:
        ...
        self.assertEqual(
            [child.title for child in toc.tree[0].children[0].children],
            ["售后服务团队", "售后服务内容及计划"],
        )
    ...
    def test_toc_generator_collapses_overdeep_tree_into_level4_leaves(self) -> None:
        ...
        leaf_titles = [child.title for child in toc.tree[0].children[0].children[0].children]
        self.assertEqual(
            leaf_titles,
            ["故障处理流程：远程诊断与现场联动", "故障处理流程：闭环整改与回访"],
        )
        self.assertTrue(
            all(node.level == 4 for node in toc.tree[0].children[0].children[0].children)
        )
